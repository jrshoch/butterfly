...
public class DumbBugPathfinder {
	...
	public static MovementAction nextStep(RobotController rc, Knowledge knowledge) {
		...
		// Change direction only if you hit map edge
		// IMPROVEMENT: randomly pick between clockwise/counterclockwise
		// IMPROVEMENT: handle robot collision
		...
		try {
			...
			System.out.println("Hocho: should be able to sense location " + nextLocation
					+ " from " + robotLocation);
			e.printStackTrace();
			return new MovementAction(Action.TURN, desiredDirection);
		}
